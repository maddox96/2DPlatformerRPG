    ...
    public class ThrownProjectile : HorizontalProjectile
    {
        ...
        protected override void Start()
        {
            ...
            if (weaponBox != null)
            {
                ...
                Projectile.IgnoreOwnerCollisions(this, this.Owner);

                Sprite.transform.localPosition = -weaponBox.offset;
                Sprite.transform.RotateAround(this.transform.position, Vector3.forward, this.StartRotation * this.Direction.x);
            }
        }
        ...
        protected override void OnCollisionEnter2D(Collision2D c)
        {
            ...
            if (this.isStopped && !this.isEndRotationSet)
            {
                ...
                {
                    float rotation = this.EndRotation - Sprite.transform.localEulerAngles.z;
                    Sprite.transform.RotateAround(this.transform.position + this.RotationPoint, Vector3.forward, rotation * this.Direction.x);
                }
            }
        }
    }
